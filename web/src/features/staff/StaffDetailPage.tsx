...
import type { StaffUpdateInput } from '@ie-platform/sdk';
...
export function StaffDetailPage() {
  ...
  useEffect(() => {
    if (editDialog.open && staffQuery.data) {
      setFormState({
        display_name: staffQuery.data.full_name ?? '',
        email: staffQuery.data.email ?? '',
        phone_number: '',
        employment_status: staffQuery.data.status ?? 'active',
      });
    }
  }, [editDialog.open, staffQuery.data]);
  ...
  return (
    <div style={{ minHeight: '100vh', padding: 32, background: theme.resolved === 'dark' ? '#0f172a' : '#f5f7fb', color: theme.resolved === 'dark' ? '#f8fafc' : '#111827' }}>
      <div style={{ maxWidth: 920, margin: '0 auto', display: 'grid', gap: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <h1 style={{ margin: '8px 0 0', fontSize: 32 }}>Staff profile</h1>
          </div>
      ...
      <Dialog open={editDialog.open} onClose={editDialog.hide} title="Edit staff" labelledBy="edit-staff-dialog">
        <form
          onSubmit={(event) => {
