...
export function AssistantUpiPaySheet({ amountPaise, onClose, onClaimed, onError }: Props) {
  ...
  async function uploadProof(file: File) {
    ...
    try {
      ...
      form.append('tags', 'assistant');
      form.set('display_name', `Assistant top-up ${paiseToInr(amountPaise)}`);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="assistant-upi-title"
      onClick={onClose}
    >
      <div className="admin-drawer admin-drawer--sheet" style={{ maxWidth: 480 }} onClick={(event) => event.stopPropagation()}>
        <p className="product-settings-kicker">Assistant wallet</p>
        <h2 id="assistant-upi-title" className="product-settings-title">
