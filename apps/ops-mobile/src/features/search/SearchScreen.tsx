...
export function SearchScreen() {
  ...
  return (
    <FormScreen>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.list}>
            {results.customers.map((item) => {
              const name = item.full_name ?? item.email ?? 'Customer';
              return (
                <ListRow
                  key={`c-${item.id}`}
                  title={name}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.list}>
            {results.staff.map((item) => {
              const name = item.full_name ?? item.email ?? 'Staff';
              return (
                <ListRow
                  key={`s-${item.id}`}
                  title={name}
      ...
      {!term ? <Text style={styles.hint}>Start typing to search across your workspace.</Text> : null}
    </FormScreen>
  );
}
...
const styles = StyleSheet.create({
  ...
  hint: { ...typography.body, color: colors.mutedForeground, textAlign: 'center', marginTop: spacing.xl },
});
